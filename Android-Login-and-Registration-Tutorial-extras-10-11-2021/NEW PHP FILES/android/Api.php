		...
		switch($_GET['apicall']){
			...
			case 'login':
				...
				if(isTheseParametersAvailable(array('username', 'password'))){
					...
					$stmt = $conn->prepare("SELECT id, username, fullname, email, gender, purok, barangay, town, meterid, totalKWH, customerbalance FROM users WHERE username = ? AND password = ?");
					...
					if($stmt->num_rows > 0){
						...
						$stmt->bind_result($id, $username, $fullname, $email, $gender, $purok, $barangay, $town, $meterid, $totalKWH, $customerbalance);
						$stmt->fetch();
