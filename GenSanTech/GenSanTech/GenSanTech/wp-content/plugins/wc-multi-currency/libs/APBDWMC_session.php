...
class APBDWMC_session {
	...
	private function __construct() {
		...
		$checkAction   = APBD_PostValue( "action", "" );
		$targethelth   = 'health-check';
		$checkHealth   = substr( $checkAction, 0, strlen( $targethelth ) );
		$isHealthCheck = $checkHealth == $targethelth;
		$baseName=!empty($_SERVER['PHP_SELF'])?basename($_SERVER['PHP_SELF']):"";
		if ( $baseName!="site-health.php" && ! session_id() && ! $this->is_rest() && ! $isHealthCheck ) {
			session_start();
		}
