jQuery(document).ready(function($) {
    'use strict';
    var this_obj = appointment_booking_plugin_activate_plugin;

    $('#gutenberg_editor .plugin-activation-redirect a').addClass('ive-redirect-to-dashboard');

    $(document).on('click', '.appointment-booking-plugin-install', function(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: function(data) {
                ...
                button.removeClass('button-secondary updating-message appointment-booking-plugin-install');
                button.addClass('button-primary appointment-booking-plugin-activate');
                button.trigger('click');
            },
            error: function(data) {
                jQuery('.appointment-booking-recommended-plugins .appointment-booking-activation-note').css('display','block');
    ...
    $(document).on('click', '.appointment-booking-plugin-activate', function(event) {
    ...
    jQuery('#lite_theme .ibtana-skip-btn').click(function(){
        ...
        jQuery('#lite_theme .appointment-booking-recommended-plugins').css('display','none');
    });

    jQuery('#block_pattern .ibtana-skip-btn').click(function(){
        var light_theme = jQuery(this).attr('get-start-tab-id');
        jQuery('.' + light_theme).css('display','block');
        jQuery('#block_pattern .appointment-booking-recommended-plugins').css('display','none');
    });
