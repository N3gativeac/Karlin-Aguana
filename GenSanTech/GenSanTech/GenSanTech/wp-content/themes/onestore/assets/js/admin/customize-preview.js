...
(function ($) {
    ...
    var onestoreCustomizer = {
        /**
         * PostMessage: Output CSS.
         */
        postMessage_css: function (key, rules, newValue) {
            var styleID = "onestore-customize-preview-css-" + key,
                $style = $("#" + styleID),
                css = "",
                cssArray = {};
                ...
                if (rule["function"] && rule["function"]["name"]) {
                    switch (rule["function"]["name"]) {
                        case "explode_value":
                            ...
                            value =
                                undefined !== array[index] ? array[index] : "";
                            break;
                ...
                if ("object" == typeof value) {
                    if (
                        ...
                        _.each(value, function (valueItem, key) {
                            concat_value.push(
                                rule["pattern"].replace("$", valueItem)
                            );
                        });
        ...
        postMessage_font: function (key, rules, newValue) {
            ...
            if (-1 < newValue.indexOf("|")) {
                ...
                if (
                    2 === chunks.length &&
                    undefined !== onestoreCustomizerPreviewData.fonts[chunks[0]]
                ) {
                    (fontSource = chunks[0]),
                        (fontName = chunks[1]),
                        (fontStack =
                            onestoreCustomizerPreviewData.fonts[fontSource][
                                fontName
                            ] || "");
                }
            }
            ...
            // Change CSS value.
            onestoreCustomizer.postMessage_css(key, rules, fontStack);

            // If value is from Google Fonts, update the embed link.
            if ("google_fonts" === fontSource) {
                // Get the <link> tag.
                var googleFontsLinkID =
                        "onestore-customize-preview-google-fonts-css-" + key,
    ...
    if (undefined !== onestoreCustomizerPreviewData) {
        if (undefined !== onestoreCustomizerPreviewData.postMessages) {
            _.each(
                onestoreCustomizerPreviewData.postMessages,
                function (postMessages, key) {
                    var sortedPostMessages = {};

                    _.each(postMessages, function (rule) {
                        var type = rule["type"];

                        if (undefined == sortedPostMessages[type])
                            sortedPostMessages[type] = [];
                    ...
                    wp.customize(key, function (setting) {
                        setting.bind(function (value) {
                            _.each(sortedPostMessages, function (rule, type) {
                                ...
                                try {
                                    onestoreCustomizer[functionName](
                                        key,
                                        rule,
                                        value
                                    );
                                    ...
                                    console.log(
                                        `Error: functionName ${functionName} not exists.`
                                    );
                                }
                            });
                        });
