...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'OneStore_Customize_Control_Dimension' ) ) :
/**
 * Dimension control class
 */
class OneStore_Customize_Control_Dimension extends OneStore_Customize_Control {
	/**
	 * @var string
	 */
	public $type = 'onestore-dimension';
	...
	protected function content_template() {
		...
		<div class="customize-control-content">
			<# _.each( data.structures, function( setting_key, device ) { #>
				<div class="onestore-dimension-fieldset onestore-row {{ data.responsive ? 'onestore-responsive-fieldset' : '' }} {{ 'desktop' == device ? 'active' : '' }} {{ 'preview-' + device }}">
					<div class="onestore-row-item" style="width: 100%;">
						<input class="onestore-dimension-input onestore-input-with-unit" type="number" value="{{ data.inputs[ setting_key ].number }}" min="{{ data.units[ data.inputs[ setting_key ].unit ].min }}" max="{{ data.units[ data.inputs[ setting_key ].unit ].max }}" step="{{ data.units[ data.inputs[ setting_key ].unit ].step }}">
					</div>
					<div class="onestore-row-item" style="width: 30px;">
						<select class="onestore-dimension-unit onestore-unit">
					...
					<input type="hidden" class="onestore-dimension-value" value="{{ data.inputs[ setting_key ].value }}" {{{ data.inputs[ setting_key ].__link }}}>
				</div>
			<# }); #>
		</div>
		<?php
...
$wp_customize->register_control_type( 'OneStore_Customize_Control_Dimension' );
endif;