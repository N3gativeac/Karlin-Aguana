...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'OneStore_Customize_Control_Dimensions' ) ) :
	/**
	 * Dimensions control class
	 */
	class OneStore_Customize_Control_Dimensions extends OneStore_Customize_Control {
		/**
		 * @var string
		 */
		public $type = 'onestore-dimensions';
		...
		<div class="customize-control-content">
			<# _.each( data.structures, function( setting_key, device ) { #>
				<div class="onestore-dimensions-fieldset onestore-row {{ data.responsive ? 'onestore-responsive-fieldset' : '' }} {{ 'desktop' == device ? 'active' : '' }} {{ 'preview-' + device }}" data-linked="false">
					<label class="onestore-row-item" style="width: 30px;">
						<span class="onestore-small-label">&nbsp;</span>
						<button class="onestore-dimensions-link button button-secondary dashicons dashicons-editor-unlink" tabindex="0"></button>
						<button class="onestore-dimensions-unlink button button-primary dashicons dashicons-admin-links" tabindex="0"></button>
					</label>
					<# _.each( [ 'top', 'right', 'bottom', 'left' ], function( prop, i ) { #>
						<label class="onestore-row-item">
							<span class="onestore-small-label">{{{ prop }}}</span>
							<input class="onestore-dimensions-input" type="number" value="{{ data.inputs[ setting_key ].numbers[ i ] }}" min="{{ data.units[ data.inputs[ setting_key ].unit ].min }}" max="{{ data.units[ data.inputs[ setting_key ].unit ].max }}" step="{{ data.units[ data.inputs[ setting_key ].unit ].step }}">
						</label>
					<# }); #>
					<label class="onestore-row-item" style="width: 30px;">
						<span class="onestore-small-label">&nbsp;</span>
					...
					<input type="hidden" class="onestore-dimensions-value" value="{{ data.inputs[ setting_key ].value }}" {{{ data.inputs[ setting_key ].__link }}}>
				</div>
			<# }); #>
		</div>
			<?php
	...
	$wp_customize->register_control_type( 'OneStore_Customize_Control_Dimensions' );
endif;
