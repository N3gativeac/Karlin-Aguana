jQuery(document).ready(function($) {
    'use strict';
    var this_obj = physiotherapy_lite_plugin_activate_plugin;

    $('#gutenberg_editor .plugin-activation-redirect a').addClass('ive-redirect-to-dashboard');

    $(document).on('click', '.physiotherapy-lite-plugin-install', function(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: function(data) {
                ...
                button.removeClass('button-secondary updating-message physiotherapy-lite-plugin-install');
                button.addClass('button-primary physiotherapy-lite-plugin-activate');
                button.trigger('click');
            },
            error: function(data) {
                jQuery('.physiotherapy-lite-recommended-plugins .physiotherapy-lite-activation-note').css('display','block');
    ...
    $(document).on('click', '.physiotherapy-lite-plugin-activate', function(event) {
    ...
    jQuery('#lite_theme .ibtana-skip-btn').click(function(){
        ...
        jQuery('#lite_theme .physiotherapy-lite-recommended-plugins').css('display','none');
    });

    jQuery('#block_pattern .ibtana-skip-btn').click(function(){
        var light_theme = jQuery(this).attr('get-start-tab-id');
        jQuery('.' + light_theme).css('display','block');
        jQuery('#block_pattern .physiotherapy-lite-recommended-plugins').css('display','none');
    });
